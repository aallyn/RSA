...
sac.model<- function(nsim,
  ...
  # For stepping through the function
  if(FALSE) {
    model.par<- read.csv("~/Desktop/rSACWinter2016/Input/SAC_batchlist_models_12122016_custom.csv", as.is = TRUE)
    ri = 151
    set.seed(ri)
    ...
    d = get(load(paste("~/Desktop/rSACWinter2016/SimulatedData/", model.par$datasets[ri], ".Rdata", sep = "")))
    datasets.list = d
