...
sac.stochastic.data<- function(nsim, 
  ...
  # For stepping through the function
  if(FALSE) {
    data.par<- read.csv("~/Dropbox/Andrew/Work/PhD/Spatial Autocorrelation/SAC_finalcountdown/ForR/SAC_batchlist_datageneration_04302016.csv", as.is = TRUE)
    ri<- 13
    nsim = data.par$nsim[ri]
    method = data.par$method[ri]
    psill.use = data.par$psill.use[ri]
    range.env1 = data.par$range.env1[ri]
    range.env2 = data.par$range.env2[ri]
    ...
    fractal.env2 = data.par$fractal.env2[ri]
    randomize = data.par$randomize[ri]
    p.rbinom = data.par$p.rbinom[ri]
    sample.size = data.par$sample.size[ri]
    census = data.par$census[ri]
    census.file = data.par$census.file[ri]
    grid.side = data.par$grid.side[ri]
    cell.size = data.par$cell.size[ri]
    logistic.env =  data.frame(suppressWarnings(cbind(a = as.numeric(data.par$logistic.env.a[ri]), b = as.numeric(data.par$logistic.env.b[ri]), c = as.numeric(data.par$logistic.env.c[ri]))))
  ...
  for(i in 1:nsim) {
    ...
    if(bio.only == TRUE) {
      ...
      names(dataset)[1:5]<- c("id", "x", "y", "x1", "zero.one")
    }
    if(exp.param != 0 && logistic.formula.length == 1) {
      dataset<- data.frame(matrix(nrow = sample.size, ncol = 6))
      names(dataset)[1:6]<- c("id", "x", "y", "x1", "x.random", "zero.one")
...
pearson.resids<- function(predicted, observed) {
  pearson<- ((predicted - observed)/sqrt(predicted*(1-predicted)))*-1
  return(pearson)
}
...
sac.model<- function(nsim,
  ...
  # For stepping through the function
  if(FALSE) {
    model.par = read.csv("~/Desktop/rSACWinter2016/SAC_batchlist_models_12122016_custom.csv", as.is = TRUE)
    model.par = model.par[model.par$model.fit == "custom.free",]
    directory = "~/Desktop/rSACWinter2016/"
    ri = 1
    set.seed(ri)
    ...
    datasets.list = get(load(paste("./Data/simulated.data/", model.par$batch[ri], ".Rdata", sep = "")))
    directory = model.par$mac.directory[1]
    ...
    negxp.functions.list = negexp.functions.list
  }
  ...
  if(datasets.group == "b") {
    datasets<- datasets.list[101:200]
  }
  if(datasets.group == "c") {
    datasets <- datasets.list[201:300]
  }
  if(datasets.group == "d") {
    datasets<- datasets.list[301:400]
  }
  if(datasets.group == "e") {
    datasets<- datasets.list[401:500]
  }
  if(datasets.group == "f") {
    datasets<- datasets.list[501:600]
  }
  if(datasets.group == "g") {
    datasets<- datasets.list[601:700]
  }
  if(datasets.group == "h") {
    datasets<- datasets.list[701:800]
  }
  if(datasets.group == "i") {
    datasets<- datasets.list[801:900]
  }
  if(datasets.group == "j") {
    datasets<- datasets.list[901:1000]
  }
  ...
  if(datasets.group == "conv.check") {
    ...
    datasets<- datasets.list[c(random.sample[1], random.sample[2])]
  }
  
  # Split list of 1000 datasets into groups of 5 for spBayes
  if(datasets.group == "a.1"){
    datasets<- datasets.list[1:5]
  }
  if(datasets.group == "a.2"){
    datasets<- datasets.list[6:10]
  }
  if(datasets.group == "a.3"){
    datasets<- datasets.list[11:15]
  }
  if(datasets.group == "a.4"){
    datasets<- datasets.list[16:20]
  }
  if(datasets.group == "a.5"){
    datasets<- datasets.list[21:25]
  }
  if(datasets.group == "a.6"){
    datasets<- datasets.list[26:30]
  }
  if(datasets.group == "a.7"){
    datasets<- datasets.list[31:35]
  }
  if(datasets.group == "a.8"){
    datasets<- datasets.list[36:40]
  }
  if(datasets.group == "a.9"){
    datasets<- datasets.list[41:45]
  }
  if(datasets.group == "a.10"){
    datasets<- datasets.list[46:50]
  }
  if(datasets.group == "a.11"){
    datasets<- datasets.list[51:55]
  }
  if(datasets.group == "a.12"){
    datasets<- datasets.list[56:60]
  }
  if(datasets.group == "a.13"){
    datasets<- datasets.list[61:65]
  }
  if(datasets.group == "a.14"){
    datasets<- datasets.list[66:70]
  }
  if(datasets.group == "a.15"){
    datasets<- datasets.list[71:75]
  }
  if(datasets.group == "a.16"){
    datasets<- datasets.list[76:80]
  }
  if(datasets.group == "a.17"){
    datasets<- datasets.list[81:85]
  }
  if(datasets.group == "a.18"){
    datasets<- datasets.list[86:90]
  }
  if(datasets.group == "a.19"){
    datasets<- datasets.list[91:95]
  }
  if(datasets.group == "a.20"){
    datasets<- datasets.list[96:100]
  }
  
  if(datasets.group == "b.1"){
    datasets<- datasets.list[101:105]
  }
  if(datasets.group == "b.2"){
    datasets<- datasets.list[106:110]
  }
  if(datasets.group == "b.3"){
    datasets<- datasets.list[111:115]
  }
  if(datasets.group == "b.4"){
    datasets<- datasets.list[116:120]
  }
  if(datasets.group == "b.5"){
    datasets<- datasets.list[121:125]
  }
  if(datasets.group == "b.6"){
    datasets<- datasets.list[126:130]
  }
  if(datasets.group == "b.7"){
    datasets<- datasets.list[131:135]
  }
  if(datasets.group == "b.8"){
    datasets<- datasets.list[136:140]
  }
  if(datasets.group == "b.9"){
    datasets<- datasets.list[141:145]
  }
  if(datasets.group == "b.10"){
    datasets<- datasets.list[146:150]
  }
  if(datasets.group == "b.11"){
    datasets<- datasets.list[151:155]
  }
  if(datasets.group == "b.12"){
    datasets<- datasets.list[156:160]
  }
  if(datasets.group == "b.13"){
    datasets<- datasets.list[161:165]
  }
  if(datasets.group == "b.14"){
    datasets<- datasets.list[166:170]
  }
  if(datasets.group == "b.15"){
    datasets<- datasets.list[171:175]
  }
  if(datasets.group == "b.16"){
    datasets<- datasets.list[176:180]
  }
  if(datasets.group == "b.17"){
    datasets<- datasets.list[181:185]
  }
  if(datasets.group == "b.18"){
    datasets<- datasets.list[186:190]
  }
  if(datasets.group == "b.19"){
    datasets<- datasets.list[191:195]
  }
  if(datasets.group == "b.20"){
    datasets<- datasets.list[196:200]
  }
  
  if(datasets.group == "c.1"){
    datasets<- datasets.list[201:205]
  }
  if(datasets.group == "c.2"){
    datasets<- datasets.list[206:210]
  }
  if(datasets.group == "c.3"){
    datasets<- datasets.list[211:215]
  }
  if(datasets.group == "c.4"){
    datasets<- datasets.list[216:220]
  }
  if(datasets.group == "c.5"){
    datasets<- datasets.list[221:225]
  }
  if(datasets.group == "c.6"){
    datasets<- datasets.list[226:230]
  }
  if(datasets.group == "c.7"){
    datasets<- datasets.list[231:235]
  }
  if(datasets.group == "c.8"){
    datasets<- datasets.list[236:240]
  }
  if(datasets.group == "c.9"){
    datasets<- datasets.list[241:245]
  }
  if(datasets.group == "c.10"){
    datasets<- datasets.list[246:250]
  }
  if(datasets.group == "c.11"){
    datasets<- datasets.list[251:255]
  }
  if(datasets.group == "c.12"){
    datasets<- datasets.list[256:260]
  }
  if(datasets.group == "c.13"){
    datasets<- datasets.list[261:265]
  }
  if(datasets.group == "c.14"){
    datasets<- datasets.list[266:270]
  }
  if(datasets.group == "c.15"){
    datasets<- datasets.list[271:275]
  }
  if(datasets.group == "c.16"){
    datasets<- datasets.list[276:280]
  }
  if(datasets.group == "c.17"){
    datasets<- datasets.list[281:285]
  }
  if(datasets.group == "c.18"){
    datasets<- datasets.list[286:290]
  }
  if(datasets.group == "c.19"){
    datasets<- datasets.list[291:295]
  }
  if(datasets.group == "c.20"){
    datasets<- datasets.list[296:300]
  }
  
  if(datasets.group == "d.1"){
    datasets<- datasets.list[301:305]
  }
  if(datasets.group == "d.2"){
    datasets<- datasets.list[306:310]
  }
  if(datasets.group == "d.3"){
    datasets<- datasets.list[311:315]
  }
  if(datasets.group == "d.4"){
    datasets<- datasets.list[316:320]
  }
  if(datasets.group == "d.5"){
    datasets<- datasets.list[321:325]
  }
  if(datasets.group == "d.6"){
    datasets<- datasets.list[326:330]
  }
  if(datasets.group == "d.7"){
    datasets<- datasets.list[331:335]
  }
  if(datasets.group == "d.8"){
    datasets<- datasets.list[336:340]
  }
  if(datasets.group == "d.9"){
    datasets<- datasets.list[341:345]
  }
  if(datasets.group == "d.10"){
    datasets<- datasets.list[346:350]
  }
  if(datasets.group == "d.11"){
    datasets<- datasets.list[351:355]
  }
  if(datasets.group == "d.12"){
    datasets<- datasets.list[356:360]
  }
  if(datasets.group == "d.13"){
    datasets<- datasets.list[361:365]
  }
  if(datasets.group == "d.14"){
    datasets<- datasets.list[366:370]
  }
  if(datasets.group == "d.15"){
    datasets<- datasets.list[371:375]
  }
  if(datasets.group == "d.16"){
    datasets<- datasets.list[376:380]
  }
  if(datasets.group == "d.17"){
    datasets<- datasets.list[381:385]
  }
  if(datasets.group == "d.18"){
    datasets<- datasets.list[386:390]
  }
  if(datasets.group == "d.19"){
    datasets<- datasets.list[391:395]
  }
  if(datasets.group == "d.20"){
    datasets<- datasets.list[396:400]
  }
  
  if(datasets.group == "e.1"){
    datasets<- datasets.list[401:405]
  }
  if(datasets.group == "e.2"){
    datasets<- datasets.list[406:410]
  }
  if(datasets.group == "e.3"){
    datasets<- datasets.list[411:415]
  }
  if(datasets.group == "e.4"){
    datasets<- datasets.list[416:420]
  }
  if(datasets.group == "e.5"){
    datasets<- datasets.list[421:425]
  }
  if(datasets.group == "e.6"){
    datasets<- datasets.list[426:430]
  }
  if(datasets.group == "e.7"){
    datasets<- datasets.list[431:435]
  }
  if(datasets.group == "e.8"){
    datasets<- datasets.list[436:440]
  }
  if(datasets.group == "e.9"){
    datasets<- datasets.list[441:445]
  }
  if(datasets.group == "e.10"){
    datasets<- datasets.list[446:450]
  }
  if(datasets.group == "e.11"){
    datasets<- datasets.list[451:455]
  }
  if(datasets.group == "e.12"){
    datasets<- datasets.list[456:460]
  }
  if(datasets.group == "e.13"){
    datasets<- datasets.list[461:465]
  }
  if(datasets.group == "e.14"){
    datasets<- datasets.list[466:470]
  }
  if(datasets.group == "e.15"){
    datasets<- datasets.list[471:475]
  }
  if(datasets.group == "e.16"){
    datasets<- datasets.list[476:480]
  }
  if(datasets.group == "e.17"){
    datasets<- datasets.list[481:485]
  }
  if(datasets.group == "e.18"){
    datasets<- datasets.list[486:490]
  }
  if(datasets.group == "e.19"){
    datasets<- datasets.list[491:495]
  }
  if(datasets.group == "e.20"){
    datasets<- datasets.list[496:500]
  }
  
  if(datasets.group == "f.1"){
    datasets<- datasets.list[501:505]
  }
  if(datasets.group == "f.2"){
    datasets<- datasets.list[506:510]
  }
  if(datasets.group == "f.3"){
    datasets<- datasets.list[511:515]
  }
  if(datasets.group == "f.4"){
    datasets<- datasets.list[516:520]
  }
  if(datasets.group == "f.5"){
    datasets<- datasets.list[521:525]
  }
  if(datasets.group == "f.6"){
    datasets<- datasets.list[526:530]
  }
  if(datasets.group == "f.7"){
    datasets<- datasets.list[531:535]
  }
  if(datasets.group == "f.8"){
    datasets<- datasets.list[536:540]
  }
  if(datasets.group == "f.9"){
    datasets<- datasets.list[541:545]
  }
  if(datasets.group == "f.10"){
    datasets<- datasets.list[546:550]
  }
  if(datasets.group == "f.11"){
    datasets<- datasets.list[551:555]
  }
  if(datasets.group == "f.12"){
    datasets<- datasets.list[556:560]
  }
  if(datasets.group == "f.13"){
    datasets<- datasets.list[561:565]
  }
  if(datasets.group == "f.14"){
    datasets<- datasets.list[566:570]
  }
  if(datasets.group == "f.15"){
    datasets<- datasets.list[571:575]
  }
  if(datasets.group == "f.16"){
    datasets<- datasets.list[576:580]
  }
  if(datasets.group == "f.17"){
    datasets<- datasets.list[581:585]
  }
  if(datasets.group == "f.18"){
    datasets<- datasets.list[586:590]
  }
  if(datasets.group == "f.19"){
    datasets<- datasets.list[591:595]
  }
  if(datasets.group == "f.20"){
    datasets<- datasets.list[596:600]
  }
  
  if(datasets.group == "g.1"){
    datasets<- datasets.list[601:605]
  }
  if(datasets.group == "g.2"){
    datasets<- datasets.list[606:610]
  }
  if(datasets.group == "g.3"){
    datasets<- datasets.list[611:615]
  }
  if(datasets.group == "g.4"){
    datasets<- datasets.list[616:620]
  }
  if(datasets.group == "g.5"){
    datasets<- datasets.list[621:625]
  }
  if(datasets.group == "g.6"){
    datasets<- datasets.list[626:630]
  }
  if(datasets.group == "g.7"){
    datasets<- datasets.list[631:635]
  }
  if(datasets.group == "g.8"){
    datasets<- datasets.list[636:640]
  }
  if(datasets.group == "g.9"){
    datasets<- datasets.list[641:645]
  }
  if(datasets.group == "g.10"){
    datasets<- datasets.list[646:650]
  }
  if(datasets.group == "g.11"){
    datasets<- datasets.list[651:655]
  }
  if(datasets.group == "g.12"){
    datasets<- datasets.list[656:660]
  }
  if(datasets.group == "g.13"){
    datasets<- datasets.list[661:665]
  }
  if(datasets.group == "g.14"){
    datasets<- datasets.list[666:670]
  }
  if(datasets.group == "g.15"){
    datasets<- datasets.list[671:675]
  }
  if(datasets.group == "g.16"){
    datasets<- datasets.list[676:680]
  }
  if(datasets.group == "g.17"){
    datasets<- datasets.list[681:685]
  }
  if(datasets.group == "g.18"){
    datasets<- datasets.list[686:690]
  }
  if(datasets.group == "g.19"){
    datasets<- datasets.list[691:695]
  }
  if(datasets.group == "g.20"){
    datasets<- datasets.list[696:700]
  }
  
  if(datasets.group == "h.1"){
    datasets<- datasets.list[701:705]
  }
  if(datasets.group == "h.2"){
    datasets<- datasets.list[706:710]
  }
  if(datasets.group == "h.3"){
    datasets<- datasets.list[711:715]
  }
  if(datasets.group == "h.4"){
    datasets<- datasets.list[716:720]
  }
  if(datasets.group == "h.5"){
    datasets<- datasets.list[721:725]
  }
  if(datasets.group == "h.6"){
    datasets<- datasets.list[726:730]
  }
  if(datasets.group == "h.7"){
    datasets<- datasets.list[731:735]
  }
  if(datasets.group == "h.8"){
    datasets<- datasets.list[736:740]
  }
  if(datasets.group == "h.9"){
    datasets<- datasets.list[741:745]
  }
  if(datasets.group == "h.10"){
    datasets<- datasets.list[746:750]
  }
  if(datasets.group == "h.11"){
    datasets<- datasets.list[751:755]
  }
  if(datasets.group == "h.12"){
    datasets<- datasets.list[756:760]
  }
  if(datasets.group == "h.13"){
    datasets<- datasets.list[761:765]
  }
  if(datasets.group == "h.14"){
    datasets<- datasets.list[766:770]
  }
  if(datasets.group == "h.15"){
    datasets<- datasets.list[771:775]
  }
  if(datasets.group == "h.16"){
    datasets<- datasets.list[776:780]
  }
  if(datasets.group == "h.17"){
    datasets<- datasets.list[781:785]
  }
  if(datasets.group == "h.18"){
    datasets<- datasets.list[786:790]
  }
  if(datasets.group == "h.19"){
    datasets<- datasets.list[791:795]
  }
  if(datasets.group == "h.20"){
    datasets<- datasets.list[796:800]
  }
  
  
  if(datasets.group == "i.1"){
    datasets<- datasets.list[801:805]
  }
  if(datasets.group == "i.2"){
    datasets<- datasets.list[806:810]
  }
  if(datasets.group == "i.3"){
    datasets<- datasets.list[811:815]
  }
  if(datasets.group == "i.4"){
    datasets<- datasets.list[816:820]
  }
  if(datasets.group == "i.5"){
    datasets<- datasets.list[821:825]
  }
  if(datasets.group == "i.6"){
    datasets<- datasets.list[826:830]
  }
  if(datasets.group == "i.7"){
    datasets<- datasets.list[831:835]
  }
  if(datasets.group == "i.8"){
    datasets<- datasets.list[836:840]
  }
  if(datasets.group == "i.9"){
    datasets<- datasets.list[841:845]
  }
  if(datasets.group == "i.10"){
    datasets<- datasets.list[846:850]
  }
  if(datasets.group == "i.11"){
    datasets<- datasets.list[851:855]
  }
  if(datasets.group == "i.12"){
    datasets<- datasets.list[856:860]
  }
  if(datasets.group == "i.13"){
    datasets<- datasets.list[861:865]
  }
  if(datasets.group == "i.14"){
    datasets<- datasets.list[866:870]
  }
  if(datasets.group == "i.15"){
    datasets<- datasets.list[871:875]
  }
  if(datasets.group == "i.16"){
    datasets<- datasets.list[876:880]
  }
  if(datasets.group == "i.17"){
    datasets<- datasets.list[881:885]
  }
  if(datasets.group == "i.18"){
    datasets<- datasets.list[886:890]
  }
  if(datasets.group == "i.19"){
    datasets<- datasets.list[891:895]
  }
  if(datasets.group == "i.20"){
    datasets<- datasets.list[896:900]
  }
  
  
  if(datasets.group == "j.1"){
    datasets<- datasets.list[901:905]
  }
  if(datasets.group == "j.2"){
    datasets<- datasets.list[906:910]
  }
  if(datasets.group == "j.3"){
    datasets<- datasets.list[911:915]
  }
  if(datasets.group == "j.4"){
    datasets<- datasets.list[916:920]
  }
  if(datasets.group == "j.5"){
    datasets<- datasets.list[921:925]
  }
  if(datasets.group == "j.6"){
    datasets<- datasets.list[926:930]
  }
  if(datasets.group == "j.7"){
    datasets<- datasets.list[931:935]
  }
  if(datasets.group == "j.8"){
    datasets<- datasets.list[936:940]
  }
  if(datasets.group == "j.9"){
    datasets<- datasets.list[941:945]
  }
  if(datasets.group == "j.10"){
    datasets<- datasets.list[946:950]
  }
  if(datasets.group == "j.11"){
    datasets<- datasets.list[951:955]
  }
  if(datasets.group == "j.12"){
    datasets<- datasets.list[956:960]
  }
  if(datasets.group == "j.13"){
    datasets<- datasets.list[961:965]
  }
  if(datasets.group == "j.14"){
    datasets<- datasets.list[966:970]
  }
  if(datasets.group == "j.15"){
    datasets<- datasets.list[971:975]
  }
  if(datasets.group == "j.16"){
    datasets<- datasets.list[976:980]
  }
  if(datasets.group == "j.17"){
    datasets<- datasets.list[981:985]
  }
  if(datasets.group == "j.18"){
    datasets<- datasets.list[986:990]
  }
  if(datasets.group == "j.19"){
    datasets<- datasets.list[991:995]
  }
  if(datasets.group == "j.20"){
    datasets<- datasets.list[996:1000]
  }
  
  # Create dataframe, "results" to store simulation results.  
  if(model.fit != "custom.fix" && model.fit != "custom.free") {
    n.predictors<- length(attr(terms(as.formula(model.formula)), "term.labels"))
    all.subsets.list<- vector("list", length = length(datasets))
    
    if(model.fit == "gam.s") {
      xyfits<- vector("list", length = length(datasets))
    }
    
    if(n.predictors == 1) {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 30))
      names(results)[1:30]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "SPEV", "SAC.var", "Time")
      results$datasets.group<- rep(datasets.group, length(datasets))
    }
    if(n.predictors == 2) {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 34))
      names(results)[1:34]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "random", "random.z", "random.p", "random.se", "deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "SPEV", "SAC.var", "Time")
      results$datasets.group<- rep(datasets.group, length(datasets))
    }
    if(n.predictors == 3) {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 38))
      names(results)[1:38]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "x2", "x2.z", "x2.p", "x2.se", "random", "random.z", "random.p", "random.se", "deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "SPEV", "SAC.var", "Time")
      results$datasets.group<- rep(datasets.group, length(datasets))
    }
    if(n.predictors == 4) {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 42))
      names(results)[1:42]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "x2", "x2.z", "x2.p", "x2.se", "dslope", "dslope.z", "dslope.p", "dslope.se", "random", "random.z", "random.p", "random.se", "deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "SPEV", "SAC.var", "Time")
      results$datasets.group<- rep(datasets.group, length(datasets))
    }
  }
  ...
  if(model.fit == "custom.fix" | model.fit == "custom.free") {
    ...
    if(custom.fit == "negexp.free") {
      ...
      results$datasets.group<- rep(datasets.group, length(datasets))
    } else if(custom.fit == "negexp.free.nox2") {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 37))
      names(results)[1:37]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "dslope", "dslope.z", "dslope.p", "dslope.se", "random", "random.z", "random.p", "random.se", "neg.exp.param","deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "Time")
      results$datasets.group<- rep(datasets.group, length(datasets))
    } else if(custom.fit == "negexp.fix.nox2") {
        results<- data.frame(matrix(nrow = length(datasets), ncol = 33))
        names(results)[1:33]<- c("intercept", "intercept.z", "intercept.p", "intercept.se", "x1", "x1.z", "x1.p", "x1.se", "random", "random.z", "random.p", "random.se", "neg.exp.param","deviance.explained", "mod.nll", "null.nll", "lrtest", "full.kappa", "full.auc", "full.ccc.mean", "full.ccc.low", "full.ccc.up", "omission.err.full", "commission.err.full", "split.kappa", "split.auc", "split.ccc.mean", "split.ccc.low", "split.ccc.up", "omission.err.split", "commission.err.split", "AICc", "Time")
        results$datasets.group<- rep(datasets.group, length(datasets))
    } else {
      results<- data.frame(matrix(nrow = length(datasets), ncol = 1))
      names(results)[1]<- c("AICc")
      results$datasets.group<- rep(datasets.group, length(datasets))
    }
  ...
  if(model.fit == "glm" || model.fit == "glm.autocov" || model.fit == "glm.rac") {
    ...
    for (k in seq(along=datasets)) {
      ...
      if(model.fit == "glm.autocov") {
        ...
        dataset.temp$x.autocov <- suppressWarnings(autocov_dist(dataset.temp$zero.one, coords, nbs = dnn.max, type = "inverse", style = "B", zero.policy = TRUE))
        ...
        training1$x.autocov <- suppressWarnings(autocov_dist(training1$zero.one, coords.t1, nbs = dnn.max, type = "inverse", style = "B", zero.policy = TRUE))
        ...
        training2$x.autocov <- suppressWarnings(autocov_dist(training2$zero.one, coords.t2, nbs = dnn.max, type = "inverse", style = "B", zero.policy = TRUE))  
      }
      ...
      if(n.predictors == 3) {
        if(any(terms.predict == "x.autocov")) {
          results$x2[k]<- "NA"
          results$x2.z[k]<- "NA"
          results$x2.p[k]<- "NA"
          results$x2.se[k]<- "NA"
      ...
      results$full.auc[k]<- PresenceAbsence::auc(DATA = dat.for.kappa.full, st.dev = FALSE)
      ...
      results$split.auc[k]<- PresenceAbsence::auc(DATA = dat.for.kappa.crossvalid, st.dev = FALSE)
      ...
      #print(paste(model.fit, k))
      syst.time.end<- Sys.time()
      results$Time[k]<- syst.time.end - syst.time.start
    }
  ...
  if(model.fit == "gam.s" || model.fit == "gam.te") {
    ...
    for (k in seq(along=datasets)) {
      ...
      if(model.subset == FALSE) {
        ...
        # How is GAM representing spatial similarity?
#         temp<- plot(mod)
#         xysmooth.plot<- temp[[1]]
#         xysmooth.x<- xysmooth.plot[["x"]]
#         xysmooth.y<- xysmooth.plot[["y"]]
#         xysmooth.dat<- data.frame(expand.grid(xysmooth.x, xysmooth.y))
#         names(xysmooth.grid)<- c("x", "y")
#         xysmooth.dat$value<- xysmooth.plot[["fit"]]
#         xyfits[[k]]<- xysmooth.dat
    ...
    ## Morans I residuals and XY Smooth data
    if(model.subset == FALSE) {
      moransI.name<- paste(directory, date, scenario, "moransI.csv", sep = "")
      #get.lock(moransI.name)
      append<- file.exists(moransI.name)
    ...
    for (k in seq(along = datasets)) {
      ...
      if(custom.fit == "negexp.fix"){
        ...
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], c = coef(glm.mod)[[3]], d = coef(glm.mod)[[4]], neg.exponent = start.params$neg.exp.param), 
      ...
      if(custom.fit == "negexp.fix.nox2"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
        
        # Training1
        points.t1<- cbind(training1$x, training1$y)
        mod.train1<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                              start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], neg.exponent = start.params$neg.exp.param), 
                              data = list(points = points.t1, x1 = training1$x1, x.random = training1$x.random, response = training1$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
        mod.train1.summary<- summary(mod.train1)
        ...
        mod.train2<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                              start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], neg.exponent = start.params$neg.exp.param), 
                              data = list(points = points.t2, x1 = training2$x1, x.random = training2$x.random, response = training2$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
        mod.train2.summary<- summary(mod.train2)
      }
      
      if(custom.fit == "negexp.fix.nox1"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], c = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x2 = dataset.temp$x2, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.fix.x1x2"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], c = coef(glm.mod)[[3]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, x2 = dataset.temp$x2, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.fix.x1xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.fix.x2xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], c = coef(glm.mod)[[2]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x2 = dataset.temp$x2, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.fix.xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.fix.xrandxnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], d = coef(glm.mod)[[2]], neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      ...
      if(custom.fit == "negexp.free"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], c = coef(glm.mod)[[3]], d = coef(glm.mod)[[4]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, x2 = dataset.temp$x2, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      ...
      if(custom.fit == "negexp.free.nox2"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      ...
      if(custom.fit == "negexp.free.nox1"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], c = coef(glm.mod)[[2]], d = coef(glm.mod)[[3]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x2 = dataset.temp$x2, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.free.x1x2"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], c = coef(glm.mod)[[3]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, x2 = dataset.temp$x2, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.free.x1xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], b = coef(glm.mod)[[2]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x1 = dataset.temp$x1, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.free.x2xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], c = coef(glm.mod)[[2]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x2 = dataset.temp$x2, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.free.xnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      
      if(custom.fit == "negexp.free.xrandxnbi"){
        ...
        points.full<- cbind(dataset.temp$x, dataset.temp$x)
        mod<- try(mle2(minuslogl = negexp.functions.list[[nll.use]],
                       start = list(a = coef(glm.mod)[[1]], d = coef(glm.mod)[[2]], e = 1, neg.exponent = start.params$neg.exp.param), 
                       data = list(points = points.full, x.random = dataset.temp$x.random, response = dataset.temp$zero.one), method = "Nelder-Mead", control=list(maxit=5000,trace=F), skip.hessian=FALSE), silent = TRUE)
      }
      ...
      # For good fits, continue
      # If model subset, just get AIC and finish
      if(custom.fit != "negexp.fix" && custom.fit != "negexp.fix.nox2" && custom.fit != "negexp.free" && custom.fit != "negexp.free.nox2") {
        results$AICc[k]<- bbmle::AICc(object = mod, nobs = sample.size)
        #print(paste(custom.fit, k))
        next
      }
      
      # If not, continue to get all the results of interest
      ...
      if(custom.fit == "negexp.fix.nox2") {
        results$random[k]<- coeffs[3,1]
        results$random.z[k]<- coeffs[3,3]
        results$random.p[k]<- coeffs[3,4]
        results$random.se[k]<- coeffs[3,2]
        results$neg.exp.param[k]<- coeffs[4,1]
      }
      ...
      if(custom.fit == "negexp.free") {
        ...
        results$random.p[k]<- coeffs[4,4]
        results$random.se[k]<- coeffs[4,2]
        results$dslope[k]<- coeffs[5,1]
        results$dslope.z[k]<- coeffs[5,3]
        results$dslope.p[k]<- coeffs[5,4]
        results$dslope.se[k]<- coeffs[5,2]
        results$neg.exp.param[k]<- coeffs[6,1]
      }
      ...
      if(custom.fit == "negexp.free.nox2") {
        ...
        results$random.p[k]<- coeffs[3,4]
        results$random.se[k]<- coeffs[3,2]
        results$dslope[k]<- coeffs[4,1]
        results$dslope.z[k]<- coeffs[4,3]
        results$dslope.p[k]<- coeffs[4,4]
        results$dslope.se[k]<- coeffs[4,2]
        results$neg.exp.param[k]<- coeffs[5,1]
      }
      
      # Calculating and storing model selection and prediction statistics
      #results$deviance.explained[k]<- (nullmod$null.deviance - deviance(mod))/nullmod$null.deviance
      #likelihood.ratio.results<- lrtest(mod, nullmod)
      #results$mod.nll[k]<- -1*likelihood.ratio.results$LogLik[1]
      #results$null.nll[k]<- -1*likelihood.ratio.results$LogLik[2]
      #results$lrtest[k]<- likelihood.ratio.results$Pr[2]
      ...
      # Model predictions
      if(custom.fit == "negexp.fix") {
        wt.mat.temp<-  exp(-results$neg.exp.param[k]*dist.mat)
        diag(wt.mat.temp)<- 0
        ind.vec.temp<- ifelse(dataset.temp$zero.one == 0, -1, 1)
        x.nbintensity.use<- as.numeric(wt.mat.temp%*%ind.vec.temp)
        fullmod.predictions<- plogis(results$intercept[k] + results$x1[k]*dataset.temp$x1 + results$x2[k]*dataset.temp$x2 + x.nbintensity.use + results$random[k]*dataset.temp$x.random)
        fullmod.resids<- dataset.temp$zero.one - fullmod.predictions
        
        mod.train1.pred<- plogis(mod.train2.summary@coef[1,1] + mod.train2.summary@coef[2,1]*training1$x1 + mod.train2.summary@coef[3,1]*training1$x2 + mod.train2.summary@coef[4,1]*training1$x.random)
        
        mod.train2.pred<- plogis(mod.train1.summary@coef[1,1] + mod.train1.summary@coef[2,1]*training2$x1 + mod.train1.summary@coef[3,1]*training2$x2 + mod.train1.summary@coef[4,1]*training2$x.random)
      }
      ...
      if(custom.fit == "negexp.fix.nox2") {
        ...
        ind.vec.temp<- ifelse(dataset.temp$zero.one == 0, -1, 1)
        x.nbintensity.use<- as.numeric(wt.mat.temp%*%ind.vec.temp)
        fullmod.predictions<- plogis(results$intercept[k] + results$x1[k]*dataset.temp$x1 + x.nbintensity.use + results$random[k]*dataset.temp$x.random)
        fullmod.resids<- dataset.temp$zero.one - fullmod.predictions
        
        mod.train1.pred<- plogis(mod.train2.summary@coef[1,1] + mod.train2.summary@coef[2,1]*training1$x1 + mod.train2.summary@coef[3,1]*training1$x.random)
        
        mod.train2.pred<- plogis(mod.train1.summary@coef[1,1] + mod.train1.summary@coef[2,1]*training2$x1 + mod.train1.summary@coef[3,1]*training2$x.random)
      }
      ...
      if(custom.fit == "negexp.free") {
        ...
        ind.vec.temp<- ifelse(dataset.temp$zero.one == 0, -1, 1)
        x.nbintensity.use<- as.numeric(wt.mat.temp%*%ind.vec.temp)
        fullmod.predictions<- plogis(results$intercept[k] + results$x1[k]*dataset.temp$x1 + results$x2[k]*dataset.temp$x2 + results$dslope[k]*x.nbintensity.use + results$random[k]*dataset.temp$x.random)
        fullmod.resids<- dataset.temp$zero.one - fullmod.predictions
        
        mod.train1.pred<- plogis(mod.train2.summary@coef[1,1] + mod.train2.summary@coef[2,1]*training1$x1 + mod.train2.summary@coef[3,1]*training1$x2 + mod.train2.summary@coef[5,1]*training1$x.random)
        
        mod.train2.pred<- plogis(mod.train1.summary@coef[1,1] + mod.train1.summary@coef[2,1]*training2$x1 + mod.train1.summary@coef[3,1]*training2$x2 + mod.train1.summary@coef[5,1]*training2$x.random)
      }
      ...
      if(custom.fit == "negexp.free.nox2") {
        ...
        ind.vec.temp<- ifelse(dataset.temp$zero.one == 0, -1, 1)
        x.nbintensity.use<- as.numeric(wt.mat.temp%*%ind.vec.temp)
        fullmod.predictions<- plogis(results$intercept[k] + results$x1[k]*dataset.temp$x1 + results$dslope[k]*x.nbintensity.use + results$random[k]*dataset.temp$x.random)
        fullmod.resids<- dataset.temp$zero.one - fullmod.predictions
        
        mod.train1.pred<- plogis(mod.train2.summary@coef[1,1] + mod.train2.summary@coef[2,1]*training1$x1 + mod.train2.summary@coef[4,1]*training1$x.random)
        
        mod.train2.pred<- plogis(mod.train1.summary@coef[1,1] + mod.train1.summary@coef[2,1]*training2$x1 + mod.train1.summary@coef[4,1]*training2$x.random)
      }
      
      dat.for.kappa.full<- data.frame(cbind(dataset.temp$id, dataset.temp$zero.one, fullmod.predictions))
      opt.thresh.full<- optimal.thresholds(DATA = dat.for.kappa.full, threshold = 50, opt.methods = "MaxKappa")
      thresh.use.full<- try(opt.thresh.full[,2], silent = TRUE)
      
      if(class(thresh.use.full) == "try-error") {
        #print("skip")
        next
      } else {
        confusion.mat.full<- cmx(DATA = dat.for.kappa.full, threshold = thresh.use.full)
        ...
        results$commission.err.full[k]<- confusion.mat.full[1,2]/sum(confusion.mat.full[,2])
        crossvalid.fulldataset.temp<- rbind(training1, training2)
        crossvalid.predictions<- c(mod.train1.pred, mod.train2.pred)
        dat.for.kappa.crossvalid<- data.frame(cbind(crossvalid.fulldataset.temp$id, crossvalid.fulldataset.temp$zero.one, crossvalid.predictions))
        opt.thresh.crossvalid<- optimal.thresholds(DATA = dat.for.kappa.crossvalid, threshold = 50, opt.methods = "MaxKappa")
        thresh.use.crossvalid<- try(opt.thresh.crossvalid[,2], silent = TRUE)
        if(class(thresh.use.crossvalid) == "try-error") {
          #print("skip")
          next
        } else {
          confusion.mat.crossvalid<- cmx(DATA = dat.for.kappa.crossvalid, threshold = thresh.use.crossvalid)
          ...
          # Morans I
          fullmod.resids<- pearson.resids(predicted = fullmod.predictions, observed = dataset.temp$zero.one)
...
#       
        ...
        if(n.predictors == 2) {
          ...
          results$SAC.var[k]<- as.numeric(mod.summary$Mean[5])
        }
        ...
        if(n.predictors == 3) {
          ...
          results$SAC.var[k]<- as.numeric(mod.summary$Mean[6])
        }
