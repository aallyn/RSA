##### All subsets using a glm2 fitting function. Modified from K. McGarigall allsubsets.glm()


all.subsets.glm2<-function(y,x,force=NULL,family=binomial(link=logit),offset=NULL,weights=NULL,
...
for(i in 1:m){
	...
	if(paired.quadratic.x){
		...
		terms <- c(svars[bin], fvars, -1)
	}
	formula <- as.formula(paste("y~", paste(terms, collapse="+"), sep=""))
	model.glm <- glm2(formula, family=family,offset=offset,weights=weights, control=list(maxit=1000), data=d)
...
all.subsets.brglm<-function(y,x,force=NULL,family=binomial(link=logit),offset=NULL,weights=NULL,
	quadratic.x=FALSE,quadratic.force=FALSE,maxp=5,select='AIC',sensitivity=.95,delta.AIC=7,
	delta.FP=.1,delta.Kappa=.1,rank=10,varimp=FALSE,gof='logLik',coef.table=FALSE, paired.quadratic.x=FALSE,...){
...
for(i in 1:m){
	...
	formula <- as.formula(paste("y~", paste(terms, collapse="+"), sep=""))
	model.glm <- brglm(formula, family=family,offset=offset,weights=weights, control.brglm=brglm.control(br.maxit=1000), data=d)
