...
function(y,x1=NULL,x2=NULL,x2.linear=FALSE,x.force=NULL,
  ...
  rank=10,varimp=FALSE,gof='logLik',coef.table=FALSE,...){
...
if(x2.linear & is.null(x2)) stop('if x2.linear=TRUE, then x2 cannot be null')
...
if(varimp==TRUE){
	...
	if(p<=12){
		require(hier.part)
		HP<-round(hier.part(y,x,family=family,gof=gof, barplot = FALSE)$I.perc/100,3)
...
concordance <-
function(use.data,avail.data,formula,n.vars,method='glm',
