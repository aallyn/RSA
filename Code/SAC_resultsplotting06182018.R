...
###########################
# Need to compare the full parameters model file to the finished fits. These are located in "~/GitHub/RSA/Data/RSA_MergedFies_Winter2018/Fits" with subfolders for Mod/Strong/Weak.
...
model.check.df<- data.frame("id" = seq(1, length(model.par.names)), "file" = model.par.names, "complete" = FALSE, "Rows" = NA)

# Need to look for a unique ID to check to see if the model run has completed.
folders<- c("Weak", "Mod", "Strong")
stem.dir<- "./Data/RSA_MergedFiles_Winter2018/Fits/"
stem.dir.out<- "./Data/RSA_MergedFiles_July2018/Fits/"
...
for (i in seq_along(folders)) {
  ...
  for(j in seq_along(all.temp)) {
    file.temp<- read.csv(paste(dir.temp, all.temp[j], sep = ""))
    file.temp$Row.Check<- seq(from = 1, to = nrow(file.temp))
    file.temp$Full.Check<- paste(file.temp$datasets.group, file.temp$Row.Check, sep = ".")
    file.temp<- file.temp[!duplicated(file.temp$Full.Check),]
    write.csv(file.temp, paste(gsub(stem.dir, stem.dir.out, dir.temp), all.temp[j], sep = ""))
...
write.csv(models.missing, "~/GitHub/RSA/Data/RSA_MergedFiles_July2018/MissingModels07102018.csv")
...
write.csv(models.missing, "~/GitHub/RSA/Inputs/MissingmodelsJuly2018.csv")
...
fit.time[,1]<- c("GLM.NO.rSAC", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
fit.time.ind<- 2

succ.rate<- data.frame(matrix(nrow = n.mods, ncol = 10))
names(succ.rate)<- c("Model", rep(c("Fine", "Med", "Coarse"), 3))
succ.rate[,1]<- c("GLM.NO.rSAC", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
succ.rate.ind<- 2
...
## Weak
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/ParamHypoPrediction/WeakSpEnv/"
...
## Mod
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/ParamHypoPrediction/ModSpEnv/"
...
## Strong
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/ParamHypoPrediction/StrongSpEnv/"
...
colors<- c('#999999', '#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00','#ffff33','#a65628','#f781bf')
lty.use<- c(1,1,2,3,5,1,1,3,1)
lty.use<- rep(1, 9)
lwd.use<- c(1.5, rep(2.5, 8))
lwd.use<- rep(2.5, 9)

out.dir<- "./Data/RSA_MergedFiles_Winter2018/"
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/MoransI/Weak/"
  ...
  # Order for plotting
  result.use<- result
  names(result.use)[24]<- "11072015emm.ewm.bm.mle2.negexp.freemoransI.csv"
  
  # Gather
  result.use<- result.use %>%
    gather(., Model, Morans.I, -dist.bin)
  
  # Clean up
  result.use$Model<- gsub("moransI.csv", "", gsub("11072015", "", result.use$Model))
  
  n<- 3
  pat<- paste0('^([^.]+(?:.[^.]+){',n-1,'}).*')
  result.use$Scenario<- sub(pat, '\\1', result.use$Model)
  result.use$Scenario.Sub<- unlist(lapply(strsplit(result.use$Scenario,  "^((?:[^.]*[.]){3}[^[.]]*)(*SKIP)(*F)|[.]", perl=TRUE), "[", 3))
  result.use$Approach<- paste(unlist(lapply(strsplit(result.use$Model,  "^((?:[^.]*[.]){2}[^[.]]*)(*SKIP)(*F)|[.]", perl=TRUE), "[", 2)), unlist(lapply(strsplit(result.use$Model,  "^((?:[^.]*[.]){2}[^[.]]*)(*SKIP)(*F)|[.]", perl=TRUE), "[", 3)), sep = ".")
  result.use$Approach<- gsub(".NA", "", result.use$Approach)
  result.use$Plot.Group<- ifelse(grepl("nox2", result.use$Approach), "no X2 model", "Global model")
  result.use$Approach<- gsub(".nox2", "", result.use$Approach)
  result.use$Approach<- ifelse(grepl(".bn", result.use$Scenario), "glm.no RSA", result.use$Approach)
  
  # Don't want to have "bn" there anymore...
  temp<- result.use[grepl(".bn", result.use$Scenario),]
  temp$Scenario<- rep(c("emm.ewf.bf", "emm.ewm.bm", "emm.ewc.bs"), each = nrow(temp)/3)
  
  result.use<- result.use[-which(grepl(".bn", result.use$Scenario)),] %>%
    bind_rows(., temp)
  
  result.use$Approach<- factor(result.use$Approach, levels = c("glm.no RSA", "glm", "autocov", "spev.negexp", "mle2.negexp.free", "gams", "gee", "spglm", "glm.wavelet"))
  result.use$Scenario<- factor(result.use$Scenario, levels = c("emm.ewf.bf", "emm.ewm.bm", "emm.ewc.bs"))
  
  # Plot
  weak<- ggplot() +
    geom_line(data = result.use, aes(x = dist.bin, y = Morans.I, color = Approach)) +
    scale_color_manual(name = "Approach", values = colors, labels = c("GLM No RSA", "GLM", "GLM+AC", "GLM+SPEV", "GLM+NBI.AC", "GAM", "GEE", "SPGLMM", "WAVE")) +
    xlim(c(0,45)) +
    xlab("Distance bin") +
    ylim(c(-0.2, 0.6)) +
    ylab("Moran's I") +
    theme_bw() +
    theme(panel.background = element_rect(fill = "#f0f0f0", colour = "black")) +
    theme(strip.background = element_rect(color = "black", fill = NA)) +
    facet_wrap(~Plot.Group+Scenario)
  ggsave(paste(out.dir, "Weak_MoransI.tiff", sep = ""), weak)
}

##### Mod species-environment relationship, fine-medium-coarse scale rSAC
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/MoransI/Mod/"
  ...
  # Don't want to have "bn" there anymore...
  temp<- result.use[grepl(".bn", result.use$Scenario),]
  temp$Scenario<- rep(c("emm.emf.bf", "emm.emm.bm", "emm.emc.bs"), each = nrow(temp)/3)
  
  result.use<- result.use[-which(grepl(".bn", result.use$Scenario)),] %>%
    bind_rows(., temp)
  
  result.use$Approach<- factor(result.use$Approach, levels = c("glm.no RSA", "glm", "autocov", "spev.negexp", "mle2.negexp.free", "gams", "gee", "spglm", "glm.wavelet"))
  result.use$Scenario<- factor(result.use$Scenario, levels = c("emm.emf.bf", "emm.emm.bm", "emm.emc.bs"))
  ...
  mod<- ggplot() +
    ...
    theme(strip.background = element_rect(color = "black", fill = NA)) +
    facet_wrap(~Plot.Group+Scenario)
  ggsave(paste(out.dir, "Mod_MoransI.tiff", sep = ""), mod)
}

##### Strong species-environment relationship, fine-medium-coarse scale rSAC
{
  dir.use<- "./Data/RSA_MergedFiles_Winter2018/MoransI/Strong/"
  ...
  # Don't want to have "bn" there anymore...
  temp<- result.use[grepl(".bn", result.use$Scenario),]
  temp$Scenario<- rep(c("emm.esf.bf", "emm.esm.bm", "emm.esc.bs"), each = nrow(temp)/3)
  
  result.use<- result.use[-which(grepl(".bn", result.use$Scenario)),] %>%
    bind_rows(., temp)
  
  result.use$Approach<- factor(result.use$Approach, levels = c("glm.no RSA", "glm", "autocov", "spev.negexp", "mle2.negexp.free", "gams", "gee", "spglm", "glm.wavelet"))
  result.use$Scenario<- factor(result.use$Scenario, levels = c("emm.esf.bf", "emm.esm.bm", "emm.esc.bs"))
  ...
  strong<- ggplot() +
    ...
    theme(strip.background = element_rect(color = "black", fill = NA)) +
    facet_wrap(~Plot.Group+Scenario)
  ggsave(paste(out.dir, "Strong_MoransI.tiff", sep = ""), strong)
}

out<- plot_grid(weak + theme(legend.position="none"), 
                strong + ylab("") + theme(axis.title.y=element_blank(),
                                          axis.text.y=element_blank(),
                                          axis.ticks.y=element_blank(), 
                                          legend.position="none"))
legend <- get_legend(weak)
out<- plot_grid(out, legend, rel_widths = c(3.3, 0.5))
ggsave(paste(out.dir, "MoransI.tiff", sep = ""), out, width = 11, height = 8, dpi = 250)
...
##### Mean bias 
{
  ...
  n.mods<- 9
  plotting.levels<- rev(c("GLM.No.RSA", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE"))
  
  mean.diff.res<- data.frame(matrix(nrow = n.mods, ncol = 10))
  names(mean.diff.res)<- c("Model", rep(c("Fine", "Med", "Coarse"), 3))
  mean.diff.res[,1]<- c("GLM.No.RSA", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
  mean.ind<- 2
  
  low.res<- data.frame(matrix(nrow = n.mods, ncol = 10))
  names(low.res)<- c("Model", rep(c("Fine", "Med", "Coarse"), 3))
  low.res[,1]<- c("GLM.No.RSA", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
  low.ind<- 2
  
  high.res<- data.frame(matrix(nrow = n.mods, ncol = 10))
  names(high.res)<- c("Model", rep(c("Fine", "Med", "Coarse"), 3))
  high.res[,1]<- c("GLM.No.RSA", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
  high.ind<- 2
  
  obs.res<- data.frame(matrix(nrow = n.mods, ncol = 10))
  names(obs.res)<- c("Model", rep(c("Fine", "Med", "Coarse"), 3))
  obs.res[,1]<- c("GLM.No.RSA", "GLM", "GLM+AC", "GLM.SPEV", "GLM+NBI.AC.FREE", "GAM", "GEE", "SPGLMM", "WAVE")
  obs.ind<- 2
  
  weak.files<- data.frame("Fine" = c("11072015emm.ewf.bn.glmglobalmodresults.csv", "11072015emm.ewf.bf.glmglobalmodresults.csv", "11072015emm.ewf.bf.autocovglobalmodresults.csv", "11072015emm.ewf.bf.spev.negexpglobalmodresults.csv", "11072015emm.ewf.bf.mle2.negexp.freeglobalmodresults.csv", "11072015emm.ewf.bf.gamsglobalmodresults.csv", "11072015emm.ewf.bf.geeglobalmodresults.csv", "11072015emm.ewf.bf.spglmglobalmodresults.csv", "11072015emm.ewf.bf.glm.waveletglobalmodresults.csv"), 
                          "Mod" =  c("11072015emm.ewm.bn.glmglobalmodresults.csv", "11072015emm.ewm.bm.glmglobalmodresults.csv", "11072015emm.ewm.bm.autocovglobalmodresults.csv", "11072015emm.ewm.bm.spev.negexpglobalmodresults.csv", "11072015emm.ewm.bm.mle2.negexp.freeglobalmodresults.csv", "11072015emm.ewm.bm.gamsglobalmodresults.csv", "11072015emm.ewm.bm.geeglobalmodresults.csv", "11072015emm.ewm.bm.spglmglobalmodresults.csv", "11072015emm.ewm.bm.glm.waveletglobalmodresults.csv"), 
                          "Coarse" = c("11072015emm.ewc.bn.glmglobalmodresults.csv", "11072015emm.ewc.bs.glmglobalmodresults.csv", "11072015emm.ewc.bs.autocovglobalmodresults.csv", "11072015emm.ewc.bs.spev.negexpglobalmodresults.csv", "11072015emm.ewc.bs.mle2.negexp.freeglobalmodresults.csv", "11072015emm.ewc.bs.gamsglobalmodresults.csv", "11072015emm.ewc.bs.geeglobalmodresults.csv", "11072015emm.ewc.bs.spglmglobalmodresults.csv", "11072015emm.ewc.bs.glm.waveletglobalmodresults.csv"))
  
  mod.files<-  data.frame("Fine" = c("11072015emm.emf.bn.glmglobalmodresults.csv", "11072015emm.emf.bf.glmglobalmodresults.csv", "11072015emm.emf.bf.autocovglobalmodresults.csv", "11072015emm.emf.bf.spev.negexpglobalmodresults.csv", "11072015emm.emf.bf.mle2.negexp.freeglobalmodresults.csv", "11072015emm.emf.bf.gamsglobalmodresults.csv", "11072015emm.emf.bf.geeglobalmodresults.csv", "11072015emm.emf.bf.spglmglobalmodresults.csv", "11072015emm.emf.bf.glm.waveletglobalmodresults.csv"), 
                          "Mod" =  c("11072015emm.emm.bn.glmglobalmodresults.csv", "11072015emm.emm.bm.glmglobalmodresults.csv", "11072015emm.emm.bm.autocovglobalmodresults.csv", "11072015emm.emm.bm.spev.negexpglobalmodresults.csv", "11072015emm.emm.bm.mle2.negexp.freeglobalmodresults.csv", "11072015emm.emm.bm.gamsglobalmodresults.csv", "11072015emm.emm.bm.geeglobalmodresults.csv", "11072015emm.emm.bm.spglmglobalmodresults.csv", "11072015emm.emm.bm.glm.waveletglobalmodresults.csv"), 
                          "Coarse" = c("11072015emm.emc.bn.glmglobalmodresults.csv", "11072015emm.emc.bs.glmglobalmodresults.csv", "11072015emm.emc.bs.autocovglobalmodresults.csv", "11072015emm.emc.bs.spev.negexpglobalmodresults.csv", "11072015emm.emc.bs.mle2.negexp.freeglobalmodresults.csv", "11072015emm.emc.bs.gamsglobalmodresults.csv", "11072015emm.emc.bs.geeglobalmodresults.csv", "11072015emm.emc.bs.spglmglobalmodresults.csv", "11072015emm.emc.bs.glm.waveletglobalmodresults.csv"))
  
  strong.files<- data.frame("Fine" = c("11072015emm.esf.bn.glmglobalmodresults.csv", "11072015emm.esf.bf.glmglobalmodresults.csv", "11072015emm.esf.bf.autocovglobalmodresults.csv", "11072015emm.esf.bf.spev.negexpglobalmodresults.csv", "11072015emm.esf.bf.mle2.negexp.freeglobalmodresults.csv", "11072015emm.esf.bf.gamsglobalmodresults.csv", "11072015emm.esf.bf.geeglobalmodresults.csv", "11072015emm.esf.bf.spglmglobalmodresults.csv", "11072015emm.esf.bf.glm.waveletglobalmodresults.csv"), 
                            "Mod" =  c("11072015emm.esm.bn.glmglobalmodresults.csv", "11072015emm.esm.bm.glmglobalmodresults.csv", "11072015emm.esm.bm.autocovglobalmodresults.csv", "11072015emm.esm.bm.spev.negexpglobalmodresults.csv", "11072015emm.esm.bm.mle2.negexp.freeglobalmodresults.csv", "11072015emm.esm.bm.gamsglobalmodresults.csv", "11072015emm.esm.bm.geeglobalmodresults.csv", "11072015emm.esm.bm.spglmglobalmodresults.csv", "11072015emm.esm.bm.glm.waveletglobalmodresults.csv"), 
                            "Coarse" = c("11072015emm.esc.bn.glmglobalmodresults.csv", "11072015emm.esc.bs.glmglobalmodresults.csv", "11072015emm.esc.bs.autocovglobalmodresults.csv", "11072015emm.esc.bs.spev.negexpglobalmodresults.csv", "11072015emm.esc.bs.mle2.negexp.freeglobalmodresults.csv", "11072015emm.esc.bs.gamsglobalmodresults.csv", "11072015emm.esc.bs.geeglobalmodresults.csv", "11072015emm.esc.bs.spglmglobalmodresults.csv", "11072015emm.esc.bs.glm.waveletglobalmodresults.csv"))
  list.files<- list(weak.files, mod.files, strong.files)
  names(list.files)<- c("Weak", "Mod", "Strong")
  
  weak.nox2.files<- data.frame("Fine" = c("11072015emm.ewf.bn.glm.nox2globalmodresults.csv", "11072015emm.ewf.bf.glm.nox2globalmodresults.csv", "11072015emm.ewf.bf.autocov.nox2globalmodresults.csv", "11072015emm.ewf.bf.spev.negexp.nox2globalmodresults.csv", "11072015emm.ewf.bf.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.ewf.bf.gams.nox2globalmodresults.csv", "11072015emm.ewf.bf.gee.nox2globalmodresults.csv", "11072015emm.ewf.bf.spglm.nox2globalmodresults.csv", "11072015emm.ewf.bf.glm.wavelet.nox2globalmodresults.csv"), 
                               "Mod" = c("11072015emm.ewm.bn.glm.nox2globalmodresults.csv", "11072015emm.ewm.bm.glm.nox2globalmodresults.csv", "11072015emm.ewm.bm.autocov.nox2globalmodresults.csv", "11072015emm.ewm.bm.spev.negexp.nox2globalmodresults.csv", "11072015emm.ewm.bm.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.ewm.bm.gams.nox2globalmodresults.csv", "11072015emm.ewm.bm.gee.nox2globalmodresults.csv", "11072015emm.ewm.bm.spglm.nox2globalmodresults.csv", "11072015emm.ewm.bm.glm.wavelet.nox2globalmodresults.csv"), 
                               "Coarse" = c("11072015emm.ewc.bn.glm.nox2globalmodresults.csv", "11072015emm.ewc.bs.glm.nox2globalmodresults.csv", "11072015emm.ewc.bs.autocov.nox2globalmodresults.csv", "11072015emm.ewc.bs.spev.negexp.nox2globalmodresults.csv", "11072015emm.ewc.bs.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.ewc.bs.gams.nox2globalmodresults.csv", "11072015emm.ewc.bs.gee.nox2globalmodresults.csv", "11072015emm.ewc.bs.spglm.nox2globalmodresults.csv", "11072015emm.ewc.bs.glm.wavelet.nox2globalmodresults.csv"))
  
  mod.nox2.files<- data.frame("Fine" = c("11072015emm.emf.bn.glm.nox2globalmodresults.csv", "11072015emm.emf.bf.glm.nox2globalmodresults.csv", "11072015emm.emf.bf.autocov.nox2globalmodresults.csv", "11072015emm.emf.bf.spev.negexp.nox2globalmodresults.csv", "11072015emm.emf.bf.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.emf.bf.gams.nox2globalmodresults.csv", "11072015emm.emf.bf.gee.nox2globalmodresults.csv", "11072015emm.emf.bf.spglm.nox2globalmodresults.csv", "11072015emm.emf.bf.glm.wavelet.nox2globalmodresults.csv"), 
                              "Mod" = c("11072015emm.emm.bn.glm.nox2globalmodresults.csv", "11072015emm.emm.bm.glm.nox2globalmodresults.csv", "11072015emm.emm.bm.autocov.nox2globalmodresults.csv", "11072015emm.emm.bm.spev.negexp.nox2globalmodresults.csv", "11072015emm.emm.bm.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.emm.bm.gams.nox2globalmodresults.csv", "11072015emm.emm.bm.gee.nox2globalmodresults.csv", "11072015emm.emm.bm.spglm.nox2globalmodresults.csv", "11072015emm.emm.bm.glm.wavelet.nox2globalmodresults.csv"), 
                              "Coarse" = c("11072015emm.emc.bn.glm.nox2globalmodresults.csv", "11072015emm.emc.bs.glm.nox2globalmodresults.csv", "11072015emm.emc.bs.autocov.nox2globalmodresults.csv", "11072015emm.emc.bs.spev.negexp.nox2globalmodresults.csv", "11072015emm.emc.bs.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.emc.bs.gams.nox2globalmodresults.csv", "11072015emm.emc.bs.gee.nox2globalmodresults.csv", "11072015emm.emc.bs.spglm.nox2globalmodresults.csv", "11072015emm.emc.bs.glm.wavelet.nox2globalmodresults.csv"))
  
  strong.nox2.files<- data.frame("Fine" = c("11072015emm.esf.bn.glm.nox2globalmodresults.csv", "11072015emm.esf.bf.glm.nox2globalmodresults.csv", "11072015emm.esf.bf.autocov.nox2globalmodresults.csv", "11072015emm.esf.bf.spev.negexp.nox2globalmodresults.csv", "11072015emm.esf.bf.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.esf.bf.gams.nox2globalmodresults.csv", "11072015emm.esf.bf.gee.nox2globalmodresults.csv", "11072015emm.esf.bf.spglm.nox2globalmodresults.csv", "11072015emm.esf.bf.glm.wavelet.nox2globalmodresults.csv"), 
                                 "Mod" = c("11072015emm.esm.bn.glm.nox2globalmodresults.csv", "11072015emm.esm.bm.glm.nox2globalmodresults.csv", "11072015emm.esm.bm.autocov.nox2globalmodresults.csv", "11072015emm.esm.bm.spev.negexp.nox2globalmodresults.csv", "11072015emm.esm.bm.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.esm.bm.gams.nox2globalmodresults.csv", "11072015emm.esm.bm.gee.nox2globalmodresults.csv", "11072015emm.esm.bm.spglm.nox2globalmodresults.csv", "11072015emm.esm.bm.glm.wavelet.nox2globalmodresults.csv"), 
                                 "Coarse" = c("11072015emm.esc.bn.glm.nox2globalmodresults.csv", "11072015emm.esc.bs.glm.nox2globalmodresults.csv", "11072015emm.esc.bs.autocov.nox2globalmodresults.csv", "11072015emm.esc.bs.spev.negexp.nox2globalmodresults.csv", "11072015emm.esc.bs.mle2.negexp.free.nox2globalmodresults.csv", "11072015emm.esc.bs.gams.nox2globalmodresults.csv", "11072015emm.esc.bs.gee.nox2globalmodresults.csv", "11072015emm.esc.bs.spglm.nox2globalmodresults.csv", "11072015emm.esc.bs.glm.wavelet.nox2globalmodresults.csv"))
  list.nox2.files<- list(weak.nox2.files, mod.nox2.files, strong.nox2.files)
  names(list.nox2.files)<- c("Weak", "Mod", "Strong")
  colors<- c("black", "#023858", "#045a8d", "#0570b0", "#3690c0", "#4daf4a", "#cb181d", "#fb6a4a", "#984ea3")
  colors<- c("#cab2d6", "#bdd7e7", "#6baed6", "#3182bd", "#08519c", "#33a02c", "#e31a1c", "#fb9a99", "#ff7f00")
  colors<- c('#999999', '#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00','#ffff33','#a65628','#f781bf')
  ## Weak
  {
    dir.use<- "./Data/RSA_MergedFiles_Winter2018/ParamHypoPrediction/WeakSpEnv/"
